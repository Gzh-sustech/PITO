...
# Loss weight
    ...
    if model_name in ['FNO', 'IFNO']:
        layers = config['model']['layers']
        width = config['model']['Width']
        modes = config['model']['modes1']
        logdir = "L{}_W{}_p{}_data{}_g{}_gp{}".format(layers, width, modes, datasize, xy_weight, cs_weight)
        print("*" * 40)
        print(f"Datasize: {datasize}  Layers: {layers}  Width: {width}  modes: {modes}")
    if model_name in ['VIT', 'IVIT']:
        layers = config['model']['n_layers']
        width = config['model']['width']
        patch_size = config['model']['patch_size']
        logdir = "L{}_W{}_P{}_data{}_g{}_gp{}".format(layers, width,patch_size,datasize, xy_weight, cs_weight)
        print("*" * 40)
        print(f"Datasize: {datasize}  Layers: {layers}  Width: {width}  Patch_size: {patch_size}")
# Iteration steps
...
# For physics-informed methods
    ...
    # Coefficent is known
        ...
        # Training process
            for e in pbar:
                if e == 0:
                    data_load_mem = get_gpu_peak_memory()
                    print(f'After data load: {data_load_mem:.2f}MB')
                t1 = default_timer()
                optimizer.zero_grad()
            # model in and out
                a,_ = next(u_loader)
                ...
                if e == 0:
                    forward_mem = get_gpu_peak_memory()
                    print(f'After forward pass: {forward_mem:.2f}MB')
            # new time series
            ...
            # back propagation
                loss.backward()
                if e == 0:
                    loss_calc_mem = get_gpu_peak_memory()
                    print(f'After loss calculation: {loss_calc_mem:.2f}MB')

                optimizer.step()
                if e == 0:
                    step_mem = get_gpu_peak_memory()
                    print(f'After optimizer step: {step_mem:.2f}MB')
                scheduler.step()
        ...
        # Training process
            ...
            # pde loss and Cs loss
                PDE_loss = PINO_loss3d(out, v, Cs_square, t_duration,flow_name)
                Cs_loss = PINO_loss3d(cs_data, v,Cs_square,  t_duration,flow_name)
            # loss function
            ...
            # evaluate step
                if e % eval_step == 0:
                    ...
                    test_loss_list.append(eval_err.item())
                    print(
                        f"Epoch: {e:05d}/{num_iter - 1}  time: {(t3 - t1):.2f}s  PDE Loss: {PDE_loss:.6f}  Cs Loss: {Cs_loss:.8f}  Cs^2: {Cs_square.item():.6f}  Test Loss: {eval_err:.6f}")
            # save step
                if e % save_step == 0 and e > satrt_step:
                    ckpt_path = os.path.join(ckpt_dir, f'model-{e}.pt')
                    save_ckpt(ckpt_path, model, optimizer, scheduler)
