...
from models import FNO3d, IFNO3d,VIT,IVIT
...
def subprocess(args):
    ...
    print("#" * 60)
    # model list
    model_name_list = ['FNO','IFNO','VIT','IVIT']
    # create model
    ...
    if model_name == model_name_list[1]:
        model = IFNO3d(layers=config['model']['layers'],
                      ...
                      act=config['model']['act'],
                      LES_coe = config['LES_model']['isKnown']).to(device)
    if model_name == model_name_list[2]:
        model = VIT(patch_size = config['model']['patch_size'],
                      ...
                      LES_coe = config['LES_model']['isKnown']).to(device)
    if model_name == model_name_list[3]:
        model = IVIT(patch_size = config['model']['patch_size'],
                      n_layers=config['model']['n_layers'],
                      width = config['model']['width'],
                      input_dim=config['model']['input_dim'],
                      output_dim=config['model']['output_dim'],
